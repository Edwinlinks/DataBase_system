<template>
  <div style="padding: 10px">
    ...
    <div style="margin: 10px 0">
      ...
      <el-upload
        action="http://localhost:9090/box/import"
    ...
    <!--    搜索区域-->
    <div style="margin: 10px 0">
      <el-input v-model="search" placeholder="请输入仓位物理地址" style="width: 20%" clearable></el-input>
...
<!--          <el-button size="mini" type="success" plain @click="showBooks(scope.row.bookList)">查看图书列表</el-button>-->
          ...
          <el-popconfirm title="确定删除吗？" @confirm="handleDelete(scope.row.boxId)">
            <template #reference>
    ...
    <el-dialog title="用户拥有的图书列表" v-model="bookVis" width="30%">
      <el-table :data="bookList" stripe border>
        <el-table-column prop="bid" label="ID"></el-table-column>
...
<!--          <el-input v-model="form.boxId" style="width: 80%"></el-input>-->
<!--        </el-form-item>-->
        <el-form-item label="仓位物理地址">
          <el-input v-model="form.boxAddr" style="width: 80%" placeholder="必填"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    handleChange(row) {
      request.put("/box/changeRole", row).then(res => {
        if (res.code === '0') {
    ...
    exportUser() {
      location.href = "http://" + window.server.filesUploadUrl + ":9090/box/export";
    },
    add() {
    ...
    save() {
      if (this.form.boxId) {  // 更新
        request.put("/box", this.form).then(res => {
          if (res.code === '0') {
            this.$message({
              type: "success",
              message: "更新成功"
        ...
        request.post("/box", this.form).then(res => {
    ...
    handleDelete(id) {
      request.delete("/box/" + id).then(res => {
        if (res.code === '0') {
          this.$message({
            type: "success",
            message: "删除成功"
